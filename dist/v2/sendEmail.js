...
const handleRaw = async (req, res) => {
    const messageId = `ses-${Math.floor(Math.random() * 900000000 + 100000000)}`;
    const message = await (0, mailparser_1.simpleParser)(Buffer.from(req.body.Content?.Raw?.Data, 'base64'));
    (0, store_1.saveEmail)({
        messageId,
...
const handleTemplate = async (req, res) => {
    ...
    const template = (0, store_1.getTemplate)(req.body.Content.Template.TemplateName);
    ...
    if (!req.body.Content?.Template?.TemplateData) {
        ...
        console.log(template.TemplateContent.Html);
    }
    const messageId = `ses-${Math.floor(Math.random() * 900000000 + 100000000)}`;
    (0, store_1.saveEmail)({
        messageId,
